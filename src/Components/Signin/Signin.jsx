...
const Signin = ({ isOpen, closeModal }) => {
  ...
  const handleOTPSend = async (e) => {
    ...
    if (result.isValid) {
      ...
      setError('')
      try {
        const response = await axios.post(`${LOGIN_BASE_URL}/Login/Createuser`, formData, {
  ...
  const OTPVerify = async (e) => {
    ...
    if (otpValue) {
      setLoading(true);
      try {
        const response = await axios.post(`${LOGIN_BASE_URL}/Login/Verify`, { userID: otpData.userID, otp: otpValue }, {
          headers: { 'Content-Type': 'application/json' }
        });
        ...
        dispatch(loginAuth());
        closeModal()
        alert.success("Welcome! Thank you for signing up. We're excited to have you on board!");
      } catch (error) {
        setError(error.response.data.messages);
        setLoading(false);
      }
  ...
  const handleOTPResend = async (e) => {
    ...
    if (result.isValid) {
      ...
      setError('')
      try {
        const response = await axios.post(`${LOGIN_BASE_URL}/Login/Signin`, formData, {
          headers: { 'Content-Type': 'application/json' }
        })
        setOtpData(response.data);
        setInput(true);
  ...
  const handleLoginSuccess = async (response) => {
    ...
    try {
      const response = await axios.post(`${LOGIN_BASE_URL}/Login/gmaillogin`, { mail: indata.email, id: indata.sub, name: indata.name, image: indata.picture }, {
        headers: { 'Content-Type': 'application/json' }
      })
      const token = response.data.token;
      localStorage.setItem('zxcvbnm@#', token);
            ...
            <form style={{ padding: 30 }}>
              <div className="form-group">
                <div className="mb-4 model-heading">
                  <strong>Property Seekers / Vendor’s Registration Form</strong>
                </div>
              </div>
              <div className="mt-3">
              ...
              <div className="form-group mt-4">
                <div className="row">
                  ...
                  {errors.mobile && <div className="validation_msg">{errors.mobile}</div>}
              ...
              >
                ...
                <div className="mt-2">
                  {timer > 0 ? (
                    ...
                    <a href="javascript:void(0)" className="text-dark" onClick={handleOTPResend}>Resend otp</a>
                  )}
                </div>
              </div>
              ...
              {input ? (
                ...
                <Link className="btn_1 mt-4 rounded full-width"
