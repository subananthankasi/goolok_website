...
const LegalpaymentInvoiceDownload = () => {
  const { id } = useParams();
  ...
  const fetchInvoice = async () => {
    try {
      const response = await axios.get(
        `${LOGIN_BASE_URL}/customer/legalpayments/${id}`
      );
        ...
        >
          ...
          >
            <div>
              ...
              <p style={{ margin: "4px 0", fontSize: "12px" }}>
                {invoiceData.email_id === "null" ? "" : invoiceData.email_id}
              </p>
            </div>
...
export default LegalpaymentInvoiceDownload;
